...
public class LayerController {
    ...
    public void enter(String action) {
        ...
        if (resetProtection && run == getLayer().getCurrentAction()) {
            return;
        }
        getLayer().setCurrentAction(action, run);
    }
    public void enter(String action, boolean loop) {
        if (anim == null) return;
        Action run = anim.action(action);
        if (resetProtection && run == getLayer().getCurrentAction()) {
            return;
        }
        getLayer().setCurrentAction(action, run, loop);
    }
    public void exit() {
        if (!isActive()) return;
    ...
    protected void createLayerIfAbsent() {
